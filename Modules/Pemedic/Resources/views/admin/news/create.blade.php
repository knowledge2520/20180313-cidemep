...
@section('content-header')
    <h1>
        {{ trans('pemedic::news.title.create new') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.new.new.index') }}">{{ trans('pemedic::news.title.news') }}</a></li>
        <li class="active">{{ trans('pemedic::news.title.create new') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.new.new.store'], 'method' => 'post', 'enctype' => 'multipart/form-data']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('pemedic::admin.news.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
